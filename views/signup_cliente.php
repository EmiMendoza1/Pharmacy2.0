<?php
// Vista de registro de clientes - Solo muestra los datos proporcionados por el controlador
// No contiene lógica de negocio ni consultas a la base de datos
// Esta vista se muestra desde la web, por lo que se cambiaría todo el diseño de aca probablemente, segun lo que uds. decidan
...
<div class="modal fade" id="modalPersona" tabindex="-1" aria-labelledby="modalPersonaLabel" aria-hidden="true">
    <div class="modal-dialog">
        ...
        <?php if (!empty($viewData['mensaje'])): ?>
            <div class="alert alert-<?= $viewData['status'] ?>" role="alert">
                <?= $viewData['mensaje'] ?>
            </div>
        <?php endif; ?>
        <form action="index.php?page=signup_cliente&action=guardar" method="POST">
            <div class="form-grid">
                ...
                <div class="input-group">
                    <label for="dni">DNI (nv)</label>
                    <input type="number" id="dni" name="dni" placeholder="Ingrese su DNI" >
                    <span class="error" id="error-dni"></span>
                </div> 
                <div class="input-group">
                    <label for="direccion">Dirección (nv)</label>
            ...
            <p>¿Ya tienes una cuenta? <a href="index.php?page=login_web">Iniciar sesión</a></p>
        </form>
    </div>
</div>
...
function agregarContacto() {
    ...
    // Asignar validación AJAX al nuevo input si es necesario
}
</script>
